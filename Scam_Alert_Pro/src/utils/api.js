...
protectedApi.interceptors.request.use(attachToken, (err) =>
  Promise.reject(err)
);
...
export const resetPassword = async (email, newPassword) => {
  const res = await authApi.post("/reset-password", { email, newPassword });
  return res.data;
};
...
export const marqueepost = async () => {
  const res = await protectedApi.get(`/marquee?ts=${Date.now()}`); 
...
export const postEmergency = async (text, files) => {
  const formData = new FormData();
  formData.append("text", text);
  ...
  const res = await protectedApi.post("/emergency", formData, {
    headers: {
      "Content-Type": "multipart/form-data",
    },
  });
...
export const fetchEmergenciesByStatus = async (status = "pending") => {
  const res = await protectedApi.get(`/emergencies?status=${status}`);
  return res.data;
};
...
export const fetchComplaintById = async (id) => {
  const res = await protectedApi.get(`/complaints/${id}`);
  return res.data;
};

export const deleteComplaint = async (complaintId) => {
  const res = await protectedApi.delete(`/complaints/${complaintId}`);
  return res.data;
};

export const deleteRepost = async (complaint_id) => {
  const res = await protectedApi.delete(`/repost/${complaint_id}`);
  return res.data;
};
...
export const fetchSavedComplaint = async () => {
  const res = await protectedApi.get("/saved-posts");
  return res.data;
};
...
export const fetchNotifications = async (page = 1, limit = 20) => {
  ...
  const res = await protectedApi.get(
    `/notifications-feed?limit=${limit}&offset=${offset}`
  );
  return res.data;
};

export const markAllNotificationsAsRead = async () => {
  const res = await protectedApi.post("/notifications-read-all");
  return res.data;
};

export const markNotificationAsRead = async (id) => {
  const res = await protectedApi.patch(`/notifications/${id}/read`);
  return res.data;
};
